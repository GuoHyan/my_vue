<template>
  <div class="add container">
    <Alert v-if="alert" v-bind:message="alert" class="alert"></Alert>
      <h4 class="page-header header">添加用户</h4>
      <form v-on:submit="addCustomer" action="">
         <div class="welll">
           <h4>用户信息</h4>
          ...
          <!-- <div class="form-group">
           ...
           <button type="submit" class="btn btn-primary submitBtn">添加</button>
...
export default {
  ...
  methods:{
    addCustomer(e){  //addCustomer同form表单里绑定的提交事件
      if(!this.customer.name||!this.customer.phone||!this.customer.email){ //判断姓名电话邮箱任意一个不能为空
          // alert("请完善相应信息!");
           this.alert="请完善相应信息!";
        }else{
          let newCustomer={  //将submit的数据存放在newCustomer对象里进行传递，以下顺序名称都和本地db.json里一致
          ...
          //将newCustomer通过post方式以url进行传递，url为（服务端）请求的地址，将添加的所有数据添加到数据里
          this.$http.post("http://localhost:3000/users",newCustomer).then(function(response){
                 // console.log(response)
                 this.$router.push({path:'/',query:{alert:"用户信息添加成功！"}})   //添加成功后跳转到主页，并添加成功后传入成功的提示，以键值对的方式
               })
               e.preventDefault();
        }
